...
export async function prepareRecommendations(config,progress=()=>{},{fetcher=fetch,expandCity=null}={}){
 ...
 if(expandCity)needs.clear(),needs.set(expandCity,Math.min(120,Math.max(40,[...places,...extra].filter(p=>p.city===expandCity).length+30)));
 for(const [city,requested] of needs){const need=Math.max(24,requested);const current=()=>[...places,...extra].filter(p=>p.city===city&&!config.visits?.some(v=>v.placeId===p.id||v.name===p.name));const capacity=()=>current().filter(p=>p.type!=='식사').length+Math.min(Math.ceil(need/5),current().filter(p=>p.type==='식사').length);if(capacity()>=need)continue;
  ...
  const [lat,lng]=stations[city],centers=[];for(const p of [...(config.stays||[]).filter(p=>p.city===city),{lat,lng},...(config.visits||[]).filter(p=>p.city===city),...current()])if(centers.length<4&&centers.every(c=>distance(c,p)>2.5))centers.push(p);
  ...
  for(const center of centers){if(capacity()>=need||requests>=8)break;
   for(const kind of (expandCity?['sights','food','cafe']:['sights','food'])){if(requests>=8)break;requests++;progress(`${city}의 ${kind==='sights'?'관광지':kind==='food'?'주변 음식점':'카페'} 후보를 확보하고 있어요…`);
