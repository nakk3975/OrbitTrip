...
http.createServer(async(req,res)=>{res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Referrer-Policy','no-referrer');res.setHeader('X-Robots-Tag','noindex, nofollow');res.setHeader('Content-Security-Policy',"default-src 'self'; style-src 'self'; script-src 'self'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'");if(req.url==='/health'){res.writeHead(200);res.end('ok');return;}if(!local){const expected='Basic '+Buffer.from(process.env.APP_USER+':'+process.env.APP_PASSWORD).toString('base64');if(!equal(req.headers.authorization||'',expected)){res.writeHead(401,{'WWW-Authenticate':'Basic realm="OrbitTrip Private", charset="UTF-8"'});res.end('Private OrbitTrip');return;}}
try{
 ...
 if(apiPath.startsWith('/api/')){
  ...
  if(apiPath==='/api/storage'&&req.method==='GET'){reply(200,{enabled:!!store});return;}
  if(!store){reply(503,{error:'서버 저장이 연결되지 않았습니다.'});return;}
  const match=apiPath.match(/^\/api\/drafts\/(JP|KR|FR|IT)$/);
